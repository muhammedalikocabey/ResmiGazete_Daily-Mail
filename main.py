...
def main():
    ...
    subscribers_mail = (client.lists.members.all('MAILCHIMP_AUDIENCE_ID', get_all=True, fields="members.email_address, members.status"))["members"]
    subscribers_status = (client.lists.members.all('MAILCHIMP_AUDIENCE_ID', get_all=True, fields="members.status"))["members"]
    subscribers_mail = [str(i["email_address"]) for i in subscribers_mail]
    subscribers_status = [str(i["status"]) for i in subscribers_status]
    
    
    all_recipients = list(zip(subscribers_mail, subscribers_status))
    
    subscribed = (list(filter(lambda x: x[1]=="subscribed", all_recipients)))
    
    recipients = [x[0] for x in subscribed]
